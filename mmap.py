...
def noncoding(genefile):
	...
	for m in c:
		m = m.rstrip().split()
		with open(os.path.join(wd+'/'+'noncoding'+'/'+'Vista_enhancers_flankinGenes.txt')) as vi:
			for v in vi:
				v = v.rstrip().split('\t')
				if m[0] == v[0] and m[1] >= v[1] and v[2] >= m[1]:
					...
					with open(p1[0]+"_regulatory-analysis.txt", 'a') as k:
						k.write('\t'.join(vista)+'\n')
		pro = []
		with open(os.path.join(wd+'/'+'noncoding'+'/'+'EPDnew_promoter.txt')) as vi:
			for v in vi:
				v = v.rstrip().split('\t')
				if m[0] == v[0] and m[1] >= v[1] and v[2] >= m[1]:
					...
					with open(p1[0]+"_regulatory-analysis.txt", 'a+') as k:
						k.write('\t'.join(pro)+'\n')
		tss= [] 
		with open(os.path.join(wd+'/'+'noncoding'+'/'+'TSS_promoter.txt')) as vi:
			for v in vi:
				v = v.rstrip().split('\t')
				if m[0] == v[0] and m[1] == v[2]:
					...
					with open(p1[0]+"_regulatory-analysis.txt", 'a+') as k:
						k.write('\t'.join(tss)+'\n')
		cpg = []	
		with open(os.path.join(wd+'/'+'noncoding'+'/'+'cpgIsland.txt')) as vi:
			for v in vi:
				v = v.rstrip().split('\t')
				if m[0] == v[0] and m[1] >= v[1] and v[2] >= m[1]:
					...
					with open(p1[0]+"_regulatory-analysis.txt", 'a+') as k:
						k.write('\t'.join(cpg)+'\n')
		ctcf = []
		with open(os.path.join(wd+'/'+'noncoding'+'/'+'insulator-CTCF-binding-sites.txt')) as vi:
			for v in vi:
				v = v.rstrip().split('\t')
				if m[0] == v[0] and m[1] >= v[1] and v[2] >= m[1]:
